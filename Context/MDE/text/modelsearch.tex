% \subsection{Model Search}z
Model search exists at the interesection of Model Driven Engineering and Artificial Intelligence.
% While most AI techniques have found applictions for this problem, 
EMF has a need for tools assisting in the modeling process.
Model search is a powerful tool with many applications.
One of the open questions in MDE is \textit{how to effectively find models that conform to a metamodel}.
When the metamodel has model constraints the problem may become complex.
...
Model search is aptly defined in \ytodo{cite Kleiner}.
It is defined as a transformation towards a model conforming to a metamodel, and starting from a partial model which conforms to a relaxation of the metamodel.

The transformation is described with the following megamodel, which is not too dissimilar to that of ATL.
\begin{figure}[!ht]
    \centering
...
Another example would be generating a unique number for the person, which satisfies a set of constraints.
...
This is particularly useful for validating metamodels.

\noindent
\textbf{Model Completion} is the process of taking a model with missing information and inferring possible information. 
...
\noindent
\textbf{Model optimization} is a transformation towards an optimal model with respect to an objective function. 
