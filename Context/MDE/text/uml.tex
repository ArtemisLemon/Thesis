...
Formally: the UML metamodel conforms to MOF meta-metamodel. \ytodo{cite}
% MOF provides a meta-metamodel similar to that presented in the previous section.

The MOF standard also describes a four layer model hierarchy illustrating its application.
...
The UML metamodel exists at the M2 layer, and the M1 layer contains the UML conforming models and metamodels created by the user to represent the system under study.
The UML languages we'll use to specify metamodels and models will are outlined in the following sections on class diagrams, object diagrams and the object constraint language.
...
class Person {  
    age : int,
    children : Person[0..*] ordered unique
}
\end{lstlisting}
Here we have a C-like or Alloy-like description of the \inlineocl{Person} metamodel.
We will commonly use \texttt{:} to express type, and \texttt{[n..m]} for collection cardinalities, and we'll use \textit{[2,3,1,1]} to describe a collections literally. 
...
\noindent \textbf{Containment} means that the \textit{contained} object is only related to one \textit{container} object through this relation.



\subsection{Object Diagrams}
% Object diagrams 
An object diagram describe instances of the concepts defined in a class diagram.
...
Below the title area, we again list the attributes and the information associated.
