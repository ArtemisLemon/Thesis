...
OCL operations generally are applied to \textit{source} OCL expressions, which can be understood as the first argument for the operation.
...
To access the parents' age, we need to navigate away from the object designated \inlineocl{self}, to the objects representing their parents, and access their age attribute. 
...
\subsubsection{Query: getting and infering ints and objects from the model}
...
\noindent \textbf{Query Expression:} \inlineocl{self.ref.select(r | predicate(r)).prop.sum()}
OCL expression resulting in a collection of integers or objects.
Can be filtered, or merged with other queries.

\subsubsection{Constraint: constraining the model}
...
\noindent \textbf{Constraint Expression:} any expression that resolves to a single boolean.

\noindent \textbf{Enforced Constraint:} any expression that must resolve to True.
...
In this theses we will be mainly focusing expressions which resolve to a CollectionType and the Integer and Boolean PrimitiveType.
...
OCL being a domain specific language in the model driven ecosystem, we naturally have a class diagram modeling the language.
