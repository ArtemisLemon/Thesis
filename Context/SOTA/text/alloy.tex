...
If we look at the upper bound for the possibilities we see tuples of atoms $\tuple{a_1,a_3}$, meaning maybe $a_1$ is linked to $a_3$.
...
\end{listing}
With the partial model from this listing, linking a single library to three books, of which 2 come from the same year, we have a very small problem to solve.
...
\subsubsection{OCLsolve}
\ytodo{cite}
OCLsolve similarly translates UML/OCL constraints into Boolean satisfiability problems, which are then solved using standard SAT solvers. 
% The library supports both the Java-based SAT4J solver and the MiniSat solver for enhanced performance on compatible systems. By relying on SAT4J, OCLsolve ensures portability across any Java Virtual Machine (JVM) environment. 
Additionally, OCLsolve leverages some components from the Kodkod solver to interface efficiently with these SAT solvers.
