...
\subsection{Global Constraints used in this thesis}
In this thesis we limit ourselves to the global constraints provided by the Choco Solver.
Some notable examples of global constraints used in this thesis are:
\begin{itemize}
    \item 
    \textcolor{black}{
    ...
    This constraint is also known as $nth$ in systems like $ECL^iPS_e$ and swi-prolog.
...
\subsection{Propagation of Element}
...
\subsection{DPLL and Simplex as Propagators}
There are notable classes of CSP which come with their own specialized algorithm to solve them.
When the variable domains are all 0/1 and the constraints in Conjunctive Normal Form (CNF), the DPLL algorithm is the best propagator.
Similarly when the variable domains are all Real numbers, and the constraints are a system of linear equations, the Simplex algorithm is the best propagator.
Traditionally, SAT and linear programming are distinct fields of symbolic AI, each with their own class for solvers. 
However from the context of constraint programming, we can understand them global constraints and propagators, and CP solvers can employ these specialized solvers.