...
\subsection{count(exp)}
Consider the expression \inlineocl{src.count()}, where \inlineocl{src} is an expression resulting in a collection and \inlineocl{exp} is an expression resulting in a single element: the \inlineocl{count} operation returns the count of \inlineocl{exp} in \inlineocl{src}.
With $X$ as the variables representing \inlineocl{src}, $y$ as the variable representing \inlineocl{exp}, and $z$ representing the count, we can define the operation as follows:
