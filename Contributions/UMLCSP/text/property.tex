...
If the domain of the problem variables is limited and order doesn't matter, this encoding can suffice to model properties. 
...
$$Class.propOcc = \{occ_{00},...,occ_{oq}\}$$
$$\forall x \in Class.propOcc,\;domain(x) = \{0..q\}$$
We similarly declare a table of variables, where $o$ is the number of objects of the class, but $q$ is the number of objects in the target class.
The domain of these variables is 
These variables are constrained to the property variables with the \textit{global cardinality constraint} $gcc$: 
$$gcc(\mathit{Class_o.property}, \mathit{Class_o.propOcc})$$
The $gcc$ constraint in applied to each object (or row) individually.
We can also use this encoding and constraint to apply set constraints, and to navigate the graph with multiplicity.