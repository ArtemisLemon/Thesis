...
% \cite{emf} 
supports UML and OCL, enabling validation of models against their meta-models and associated constraints. It also includes model transformation tools such as ATL~\cite{jouault2008atl,Le_Calvar2019-atol}, an OCL-based language that expresses mappings between meta-models.

The Object Constraint Language 
(OCL)
\footnote{\url{https://www.omg.org/spec/OCL/2.4/}} is a popular language in Model-Driven Engineering (MDE) to define constraints on models and metamodels. OCL invariants are commonly used to express and validate model correctness. 
% ,jackson_alcoa_2000
...
The field of constraint based validation and solving on EMF models, logical and constraint solvers have been leveraged to validate UML metamodels with OCL constraints, used by tools like Viatra Solver \cite{ViatraSolver}, EMF2CSP \cite{Gonzalez2012-emf2csp} and Alloy~\cite{Jackson2002-alloy} in its own right and via UML2Alloy\cite{uml2alloy} and Viatra Solver.
...
constraints is crucial for performance of constraint enforcement on
models.
...
% Previous work 
% by Le Calvar et al. 
%, found though \ynote{deterministic queries}{deterministic isn't the word} what we will later define as constant queries.
...
Related work leveraging CP, global constraints and similar models also exists \cite{ferdjoukh_instantiation_2015}, however UML/OCL coverage doesn't include the general case of collection properties discussed in this paper, and required for the experimentation. EMFtoCSP\cite{Gonzalez2012-emf2csp} also leverages global constraints and has good OCL coverage, but doesn't propagate over the different sizes of collections.
Nether of these tools are included in experimentation however, as we didn't manage to get them working in time.



In this thesis extend coverage of OCL for \atlc.
ATLc~\cite{Le_Calvar2021-solvers} extends ATL by introducing model space exploration capabilities to facilitate transformation specification. It leverages constraint solvers to generate and visualize model instances, which users can then adjust or repair using solver feedback.
\textcolor{blue}{
...
However, ATLc is currently limited to arithmetic over single-valued model attributes, using integers or reals.
\textcolor{blue}{
Our work seeks to generalize this approach to support collection-valued properties: attributes and relations.
}
% \atlc has started to bridge from arithmetic OCL constraints to arithmetic constraint programming models, but it exclusively focused on constraints over integer attributes.
...
we describe a general CP method for enforcing annotated structural OCL constraints, i.e. constraints predicating on navigation chains. 
