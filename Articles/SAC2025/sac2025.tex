...
\label{sec:sac2025:intro}
% \ytodo{with \autoref{sec:sac2025:motiv} 1 1/2 pages}
%\begin{outline}
...
The paper is structured as follows. In \autoref{sec:sac2025:motiv} we present a problem on a UML model as our running case. In \autoref{sec:sac2025:contrib} we describe how we annotate CP variables in OCL.  
In \autoref{sec:sac2025:contrib2} we show how we model the annotated UML and OCL problem as a CSP.
% In \autoref{sec:sac2025:contrib} we present our contributions. a standard for identifying OCL variables to give to solvers,
...
% In \autoref{sec:sac2025:impl} we describe our implementation over the ATLc flavor of OCL.
In \autoref{sec:sac2025:eval} we determine the size and performance of the resulting CP model.
In \autoref{sec:sac2025:disc} we discuss limitations and future work.
In \autoref{sec:sac2025:related} we look at related work.
Finally, we conclude in \autoref{sec:sac2025:qed}.
...
\section{Background and Running Case} \label{sec:sac2025:motiv} 
% \ytodo{with \autoref{sec:sac2025:intro} 1 1/2 pages}
...
\subsection{Constraint Programming} \label{ssec:sac2025: csp_intro}
% \stodo{
...
\section{Denoting CP Variables in OCL Expressions}\label{sec:sac2025:contrib}\label{ssec:sac2025:varop}
...
In a second step, described in Section~\ref{sec:sac2025:contrib2}, the resulting annotated OCL will be translated to a CP model. 
...
\label{sec:sac2025:contrib2}

To enforce OCL constraints on existing model instances using CP, we must encode parts of the model instances as CSP variables and constraints (\autoref{subsec:sac2025:assoc}), and then encode the OCL using additional CSPs on the variables of the model instances (\autoref{subsec:sac2025:expr}). 

\subsection{References in CP}
\label{subsec:sac2025:assoc}
...
\label{sssec:sac2025:adjlistnav}
...
In \autoref{ssec:sac2025:navCSPeval} we will count the number of query atoms to evaluate the query model.
% \ytodo{more?}
...
To build the OCL CSP, for each \texttt{Task} we start from the bottom of its tree, upon reaching an annotated node, we get the UML CSP variables modeling that property, the \texttt{variable property access} in \autoref{fig:compilation} described in \autoref{sssec:sac2025:erefnav}.
In this case the accessed property is a reference, allowing us to navigate further.
Navigating from a variable reference modeled with pointers means using \autoref{csp:nav} described in \autoref{sssec:sac2025:adjlistnav}.
...
As hinted as in \autoref{ssec:sac2025:refactor}, some OCL refactoring seems to be systematically applicable.
...
\section{Related Work} \label{sec:sac2025:related}
...
\section{Conclusion}\label{sec:sac2025:qed} 
% \ytodo{1 page with \autoref{sec:sac2025:related}}
